...
class ArrayUtilsTests : StringSpec() {
  init {
    ...
    // Elements use the platform's toString, and JS prints a whole-number Float or Double without ".0".
    "float array test" {
      ArrayUtils.asString(floatArrayOf()) shouldBe "[]"
      ArrayUtils.asString(floatArrayOf(1.0f)) shouldBe if (testPlatform == TestPlatform.JS) "[1]" else "[1.0]"
      ArrayUtils.asString(floatArrayOf(1.5f, 2.5f, 3.5f)) shouldBe "[1.5, 2.5, 3.5]"
    }

    "double array test" {
      ArrayUtils.asString(doubleArrayOf()) shouldBe "[]"
      ArrayUtils.asString(doubleArrayOf(1.0)) shouldBe if (testPlatform == TestPlatform.JS) "[1]" else "[1.0]"
