...
- [ ] **CR-002** `MEDIUM` · bug — Blocklist prefix `java.lang.Runtime` rejects every `RuntimeException` subclass ([details](#cr-002)) — in progress (branch `fix-core-utils-review-items`)
- [ ] **CR-003** `MEDIUM` · bug — `maskUrlCredentials` fabricates credentials/host when `@` is in path or query ([details](#cr-003)) — in progress (branch `fix-core-utils-review-items`)
- [ ] **CR-004** `MEDIUM` · bug — Glob→regex (`toPattern`/`asRegex`) escapes only `.` ([details](#cr-004)) — in progress (branch `fix-core-utils-review-items`)
- [ ] **CR-005** `MEDIUM` · bug — `Long.length` off by one for values near 10^16–10^18 ([details](#cr-005)) — in progress (branch `fix-core-utils-review-items`)
- [ ] **CR-006** `MEDIUM` · bug — `toISO8601` drops seconds on round minutes ([details](#cr-006)) — in progress (branch `fix-core-utils-review-items`)
- [ ] **CR-007** `MEDIUM` · bug — Two-digit years computed as `year - 2000`; `versionDesc` fallback prints `12/31/-31` ([details](#cr-007)) — in progress (branch `fix-core-utils-review-items`)
- [ ] **CR-008** `MEDIUM` · bug — `singleSetReference` compares by reference; non-null `initValue` makes it unsettable ([details](#cr-008)) — in progress (branch `fix-core-utils-review-items`)
- [ ] **CR-009** `MEDIUM` · bug — `typeParameterCount` counts the superclass's type args (breaks script-utils `add()`) ([details](#cr-009)) — in progress (branch `fix-core-utils-review-items`)
- [ ] **CR-010** `MEDIUM` · bug — `ContentRoot.file(path)` ignores the root ([details](#cr-010)) — in progress (branch `fix-core-utils-review-items`)
- [ ] **CR-011** `MEDIUM` · bug — `GitLabFile` targets the HTML `/-/blob/` page instead of `/-/raw/` ([details](#cr-011)) — in progress (branch `fix-core-utils-review-items`)
