...
### email-utils
- [x] **CR-099** `MEDIUM` · bug — Webhook models can't decode real Resend payloads with a default `Json` ([details](#cr-099)) — fixed in #183
- [x] **CR-100** `LOW` · API — Default `email()` stylesheet exists only in test resources ([details](#cr-100)) — fixed in #183
- [x] **CR-101** `LOW` · API — `Parameters.getEmail` doesn't normalize (carried over) ([details](#cr-101)) — fixed in #183
- [x] **CR-102** `LOW` · logging — `sendEmail` logs-then-rethrows; logs recipient addresses at INFO (partly carried over) ([details](#cr-102)) — fixed in #183

### recaptcha-utils
- [x] **CR-103** `MEDIUM` · concurrency — `verifyRecaptcha` swallows `CancellationException` ([details](#cr-103)) — fixed in #183
- [x] **CR-104** `LOW` · bug — `remoteip` sends a reverse-DNS hostname ([details](#cr-104)) — fixed in #183
- [x] **CR-105** `LOW` · security — Enabled-but-misconfigured fails open silently; duplicated dead gate (carried over) ([details](#cr-105)) — fixed in #183

### redis-utils
- [x] **CR-106** `MEDIUM` · bug — `withRedis` family never takes its documented connection-failure path ([details](#cr-106)) — fixed in #183
- [x] **CR-107** `MEDIUM` · bug — URL parsing drops the database index and protocol ([details](#cr-107)) — fixed in #183
- [x] **CR-108** `LOW` · bug — `"none"` placeholder password still sent as AUTH (carried over) ([details](#cr-108)) — fixed in #183
- [x] **CR-109** `LOW` · error handling — Pool helpers catch only `JedisConnectionException` ([details](#cr-109)) — fixed in #183
- [x] **CR-110** `LOW` · performance — `testOnBorrow` + `testOnReturn` add two PINGs per command ([details](#cr-110)) — fixed in #183
- [x] **CR-111** `LOW` · security — SSL scheme detection uses default-locale `lowercase` ([details](#cr-111)) — fixed in #183
- [x] **CR-112** `LOW` · API — Pool-size validation rejects `-1`, accepts `0` ([details](#cr-112)) — fixed in #183

### exposed-utils
- [x] **CR-113** `MEDIUM` · bug — `ResultRow.get(index)` / `toRowString()` throw on SQL NULL ([details](#cr-113)) — fixed in #183
...
- [ ] **CR-125** `MEDIUM` · publishing — grpc-utils (Netty `SslContext`) and script-utils-java (`Isolation`) leak `implementation` types ([details](#cr-125)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-126** `LOW` · deps — Catalog `kotlin` entry also pins `kotlin-reflect`; POMs mix 2.4.10 / 2.4.20 ([details](#cr-126)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-127** `LOW` · deps — Redundant/unused dependency declarations ([details](#cr-127)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-128** `LOW` · build — `make build` ("without tests") still runs KMP `jvmTest` via Kover ([details](#cr-128)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-129** `LOW` · build — `coverage-clean` misses JVM-module test results; build cache restores them ([details](#cr-129)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-130** `LOW` · build — `ksp` plugin not declared in root `plugins {}` ([details](#cr-130)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-131** `LOW` · publishing — Signing skipped unless in-memory key present; POM `developerConnection` lacks `git@` ([details](#cr-131)) — in progress (branch `fix-review-items-cr125-cr139`)

### CI
- [ ] **CR-132** `MEDIUM` · security — `test.yml` has no `permissions` block; repo default token is write ([details](#cr-132)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-133** `LOW` · security — `kdocs.yml` grants `pages`/`id-token` write to the PR build job ([details](#cr-133)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-134** `LOW` · ci — Redundant Lint step; Apple targets never built in CI ([details](#cr-134)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-135** `LOW` · ci — No concurrency cancellation; actions pinned by mutable tags ([details](#cr-135)) — in progress (branch `fix-review-items-cr125-cr139`)

### Project documentation
- [ ] **CR-136** `MEDIUM` · docs — llms.txt misdescribes several modules ([details](#cr-136)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-137** `LOW` · docs — CLAUDE.md format command skips KMP modules ([details](#cr-137)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-138** `LOW` · docs — Other CLAUDE.md drift (Dokka location, return-value checker, Dependabot ignores) ([details](#cr-138)) — in progress (branch `fix-review-items-cr125-cr139`)
- [ ] **CR-139** `LOW` · cleanup — Stale/duplicated top-level files (old reviews, `.codeclimate.yml`, `system.properties`, `.wercker/`) ([details](#cr-139)) — in progress (branch `fix-review-items-cr125-cr139`)
