...
- [ ] **CR-081** `MEDIUM` · concurrency — Pooled engines lost forever when a waiting borrower is cancelled ([details](#cr-081)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-082** `MEDIUM` · security — Expression evaluators and their pools have no JVM-exit guard ([details](#cr-082)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-083** `MEDIUM` · bug — Auto-imported `System` shadow breaks all other `System.*` calls in Kotlin scripts ([details](#cr-083)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-084** `MEDIUM` · bug — `KotlinScript` generates uncompilable casts (`Regex`, `listOf`, nested generics) ([details](#cr-084)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-085** `MEDIUM` · leak — `JavaScript` imports/isolation leak across pool borrowers and grow unbounded ([details](#cr-085)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-086** `MEDIUM` · leak — Kotlin expression evaluator pools never reset; REPL history grows forever ([details](#cr-086)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-087** `MEDIUM` · bug — Variables added after first `eval` never bound (Java silently returns default); `add()` unsynchronized (carried over, extended) ([details](#cr-087)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-088** `LOW` · security — Python guard blocks harmless `sys.exit` but misses `java.lang.System.exit` ([details](#cr-088)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-089** `LOW` · docs — Python guard KDoc inverted about string literals; `def exit(self)` rejected ([details](#cr-089)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-090** `LOW` · robustness — Pools accept `size <= 0` and hang; `AbstractExprEvaluatorPool<T>` ignores `T` ([details](#cr-090)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-091** `LOW` · leak — Pools never close their instances ([details](#cr-091)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-092** `LOW` · concurrency — Expression evaluators share the manager's global `Bindings` ([details](#cr-092)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-093** `LOW` · security — Binding names unvalidated and spliced raw into generated source ([details](#cr-093)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-094** `LOW` · bug — `javaEquiv` emits invalid Java for `Char`, `Any`, nested generics (carried over) ([details](#cr-094)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-095** `LOW` · bug — `JavaScript.varDecls` uses runtime `simpleName`; raw `IllegalArgumentException` escapes (carried over) ([details](#cr-095)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-096** `LOW` · docs — No-op `close()`, undocumented `eval()`, undocumented `evalScript` field contract (carried over) ([details](#cr-096)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-097** `LOW` · cleanup — Public `engine`, `toTempName`, dead `error()`, duplicated binding flush (carried over) ([details](#cr-097)) — in progress (branch `fix-review-items-cr081-cr098`)
- [ ] **CR-098** `LOW` · tests — Misleading guard tests; missing coverage for the defects above ([details](#cr-098)) — in progress (branch `fix-review-items-cr081-cr098`)
