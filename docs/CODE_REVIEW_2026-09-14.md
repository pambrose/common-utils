...
- [ ] **CR-012** `LOW` · docs — core-utils README snippets don't compile or misdescribe behavior ([details](#cr-012)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-013** `LOW` · robustness — `UrlSource.content`: no timeouts, refetch on every access, deprecated `URL(String)` ([details](#cr-013)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-014** `LOW` · bug — `join`/`toPath`: multi-char separators and empty elements ([details](#cr-014)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-015** `LOW` · bug — `GitHubRepo.rawSourcePrefix` replace-all and custom-domain inconsistency ([details](#cr-015)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-016** `LOW` · security/docs — Unkeyed SHA-256 checksum billed as tamper detection; magic `32` ([details](#cr-016)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-017** `LOW` · API — `toByteArraySecure` duplicates `toByteArray`; `ReplaceWith` hints don't compile ([details](#cr-017)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-018** `LOW` · bug — `getBanner`/`ReadResources` use the library's classloader ([details](#cr-018)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-019** `LOW` · bug — `readProperties` isn't `.properties`-compatible and applies files partially ([details](#cr-019)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-020** `LOW` · API — `waitForPortAvailable` gives up silently; `repeatWithSleep` sleeps after last iteration ([details](#cr-020)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-021** `LOW` · cleanup — `hostInfo` double lookup, `!!`, commented-out code (carried over) ([details](#cr-021)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-022** `LOW` · docs — `captureStdout` swaps global `System.out`, default charset (carried over) ([details](#cr-022)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-023** `LOW` · docs — `toCsv` performs no escaping, undocumented (carried over) ([details](#cr-023)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-024** `LOW` · API — `atomicInteger`/`atomicLong` default to -1; dead commented-out delegate (carried over) ([details](#cr-024)) — in progress (branch `fix-core-utils-low-items`)
- [ ] **CR-025** `LOW` · tests — Boundary tests missing, weak assertions, 10M-iteration sweep in `commonTest` ([details](#cr-025)) — in progress (branch `fix-core-utils-low-items`)

### json-utils
- [ ] **CR-026** `MEDIUM` · API — `s.toJsonString()` and `s.toJsonString(prettyPrint = true)` do different things ([details](#cr-026))
- [ ] **CR-027** `MEDIUM` · bug — Non-`OrNull` accessors turn JSON `null` into `"null"` / `false` ([details](#cr-027))
- [ ] **CR-028** `MEDIUM` · bug/docs — `*OrNull` accessors throw on type mismatch, contrary to README ([details](#cr-028))
- [ ] **CR-029** `LOW` · bug — `booleanValue` treats any non-`"true"` as `false` (carried over) ([details](#cr-029))
- [ ] **CR-030** `LOW` · API — `isNumber` true for quoted numeric strings (carried over) ([details](#cr-030))
- [ ] **CR-031** `LOW` · API — Internal `JsonElementUtils` logger holder is public (carried over) ([details](#cr-031))
- [ ] **CR-032** `LOW` · API — Path navigation: inconsistent empty segments; dotted keys unreachable (carried over) ([details](#cr-032))
- [ ] **CR-033** `LOW` · docs — README examples missing `get` import; `size` on an array throws ([details](#cr-033))
- [ ] **CR-034** `LOW` · docs — `isEmpty()` KDoc says "blank"; `JsonNull` counts as non-empty ([details](#cr-034))
- [ ] **CR-035** `LOW` · bug — `deepCopy()` string round-trip throws on NaN/Infinity ([details](#cr-035))
- [ ] **CR-036** `LOW` · tests — json-utils tests that cannot fail or are unfinished ([details](#cr-036))
...
- [ ] **CR-042** `LOW` · bug — Servlet character encoding ignored by writer and Content-Type (carried over) ([details](#cr-042))
- [ ] **CR-043** `LOW` · docs — Parameter lookups case-insensitive; comment contradicts `getParameterMap` ([details](#cr-043))
- [ ] **CR-044** `LOW` · bug/tests — `excludeSuffix`/`excludePrefix` match query string; redirect tests assert status only ([details](#cr-044))

### service-utils
- [ ] **CR-045** `MEDIUM` · bug — Jetty admin/metrics paths with a leading slash become `//ping` → 404 (carried over) ([details](#cr-045))
- [ ] **CR-046** `MEDIUM` · leak — Failed `startUp()`/`shutDown()` leaves already-started sub-services running ([details](#cr-046))
- [ ] **CR-047** `MEDIUM` · tests — No test issues an HTTP request to any admin/metrics endpoint ([details](#cr-047))
- [ ] **CR-048** `LOW` · API — Services added after init are silently unmanaged; test asserts the wrong thing ([details](#cr-048))
- [ ] **CR-049** `LOW` · bug — `ZipkinReporterService.shutDown` drops queued spans (no `flush()`) ([details](#cr-049))
- [ ] **CR-050** `LOW` · leak — `DropwizardExports` registered globally, never unregistered ([details](#cr-050))
- [ ] **CR-051** `LOW` · robustness — Failure logged at info; Zipkin URL `//`; no init guard (carried over) ([details](#cr-051))
- [ ] **CR-052** `LOW` · API — `ZipkinConfig.serviceName` never used (carried over) ([details](#cr-052))
- [ ] **CR-053** `LOW` · security — Admin and metrics servers bind all interfaces, no host setting ([details](#cr-053))
- [ ] **CR-054** `LOW` · cleanup — `runBlocking` around non-suspend `start`; single-use `servletGroup` lateinits ([details](#cr-054))

### prometheus-utils
- [ ] **CR-055** `MEDIUM` · API — Factories hard-wired to the default `CollectorRegistry` (carried over) ([details](#cr-055))
- [ ] **CR-056** `LOW` · robustness — `SystemMetrics.initialize` unrecoverable after partial registration ([details](#cr-056))
- [ ] **CR-057** `LOW` · bug — `SamplerGaugeCollector` runs the sampler during construction ([details](#cr-057))
- [ ] **CR-058** `LOW` · bug — `InstrumentedThreadFactory` counts before null check; misleading comment (carried over) ([details](#cr-058))
- [ ] **CR-059** `LOW` · docs/tests — README `HTTPServer` example needs an unlisted artifact; weak metric tests ([details](#cr-059))

### dropwizard-utils
- [ ] **CR-060** `LOW` · API — `newBacklogHealthCheck` captures a snapshot; message omits threshold; no `@JvmStatic` (carried over) ([details](#cr-060))

### jetty-utils
- [ ] **CR-061** `LOW` · bug — `LambdaServlet` returns 200/empty when the lambda throws (carried over) ([details](#cr-061))
- [ ] **CR-062** `LOW` · bug — `text/plain` without charset is encoded ISO-8859-1 on Jetty ([details](#cr-062))
- [ ] **CR-063** `LOW` · cleanup — `VersionServlet` duplicates `LambdaServlet`; DSL builders lack default block; `HttpServletGroup` untested (carried over) ([details](#cr-063))
...
- [ ] **CR-082** `MEDIUM` · security — Expression evaluators and their pools have no JVM-exit guard ([details](#cr-082))
- [ ] **CR-083** `MEDIUM` · bug — Auto-imported `System` shadow breaks all other `System.*` calls in Kotlin scripts ([details](#cr-083))
- [ ] **CR-084** `MEDIUM` · bug — `KotlinScript` generates uncompilable casts (`Regex`, `listOf`, nested generics) ([details](#cr-084))
- [ ] **CR-085** `MEDIUM` · leak — `JavaScript` imports/isolation leak across pool borrowers and grow unbounded ([details](#cr-085))
- [ ] **CR-086** `MEDIUM` · leak — Kotlin expression evaluator pools never reset; REPL history grows forever ([details](#cr-086))
- [ ] **CR-087** `MEDIUM` · bug — Variables added after first `eval` never bound (Java silently returns default); `add()` unsynchronized (carried over, extended) ([details](#cr-087))
- [ ] **CR-088** `LOW` · security — Python guard blocks harmless `sys.exit` but misses `java.lang.System.exit` ([details](#cr-088))
- [ ] **CR-089** `LOW` · docs — Python guard KDoc inverted about string literals; `def exit(self)` rejected ([details](#cr-089))
- [ ] **CR-090** `LOW` · robustness — Pools accept `size <= 0` and hang; `AbstractExprEvaluatorPool<T>` ignores `T` ([details](#cr-090))
- [ ] **CR-091** `LOW` · leak — Pools never close their instances ([details](#cr-091))
- [ ] **CR-092** `LOW` · concurrency — Expression evaluators share the manager's global `Bindings` ([details](#cr-092))
- [ ] **CR-093** `LOW` · security — Binding names unvalidated and spliced raw into generated source ([details](#cr-093))
- [ ] **CR-094** `LOW` · bug — `javaEquiv` emits invalid Java for `Char`, `Any`, nested generics (carried over) ([details](#cr-094))
- [ ] **CR-095** `LOW` · bug — `JavaScript.varDecls` uses runtime `simpleName`; raw `IllegalArgumentException` escapes (carried over) ([details](#cr-095))
- [ ] **CR-096** `LOW` · docs — No-op `close()`, undocumented `eval()`, undocumented `evalScript` field contract (carried over) ([details](#cr-096))
- [ ] **CR-097** `LOW` · cleanup — Public `engine`, `toTempName`, dead `error()`, duplicated binding flush (carried over) ([details](#cr-097))
- [ ] **CR-098** `LOW` · tests — Misleading guard tests; missing coverage for the defects above ([details](#cr-098))
