...
- [ ] **CR-012** `LOW` · docs — core-utils README snippets don't compile or misdescribe behavior ([details](#cr-012)) — in progress (#176)
- [ ] **CR-013** `LOW` · robustness — `UrlSource.content`: no timeouts, refetch on every access, deprecated `URL(String)` ([details](#cr-013)) — in progress (#176)
- [ ] **CR-014** `LOW` · bug — `join`/`toPath`: multi-char separators and empty elements ([details](#cr-014)) — in progress (#176)
- [ ] **CR-015** `LOW` · bug — `GitHubRepo.rawSourcePrefix` replace-all and custom-domain inconsistency ([details](#cr-015)) — in progress (#176)
- [ ] **CR-016** `LOW` · security/docs — Unkeyed SHA-256 checksum billed as tamper detection; magic `32` ([details](#cr-016)) — in progress (#176)
- [ ] **CR-017** `LOW` · API — `toByteArraySecure` duplicates `toByteArray`; `ReplaceWith` hints don't compile ([details](#cr-017)) — in progress (#176)
- [ ] **CR-018** `LOW` · bug — `getBanner`/`ReadResources` use the library's classloader ([details](#cr-018)) — in progress (#176)
- [ ] **CR-019** `LOW` · bug — `readProperties` isn't `.properties`-compatible and applies files partially ([details](#cr-019)) — in progress (#176)
- [ ] **CR-020** `LOW` · API — `waitForPortAvailable` gives up silently; `repeatWithSleep` sleeps after last iteration ([details](#cr-020)) — in progress (#176)
- [ ] **CR-021** `LOW` · cleanup — `hostInfo` double lookup, `!!`, commented-out code (carried over) ([details](#cr-021)) — in progress (#176)
- [ ] **CR-022** `LOW` · docs — `captureStdout` swaps global `System.out`, default charset (carried over) ([details](#cr-022)) — in progress (#176)
- [ ] **CR-023** `LOW` · docs — `toCsv` performs no escaping, undocumented (carried over) ([details](#cr-023)) — in progress (#176)
- [ ] **CR-024** `LOW` · API — `atomicInteger`/`atomicLong` default to -1; dead commented-out delegate (carried over) ([details](#cr-024)) — in progress (#176)
- [ ] **CR-025** `LOW` · tests — Boundary tests missing, weak assertions, 10M-iteration sweep in `commonTest` ([details](#cr-025)) — in progress (#176)
